...
export default class ArticleController {
  ...
  static async saveArticle(req, res) {
    ...
    const coverUrl = req.files && req.files[0] !== undefined && req.files[0].originalname
      ? `v${req.files[0].version}/${req.files[0].public_id}.${req.files[0]
        .format}`
      : null;
    const article = await Article.create({
      userId: req.user.id,
      slug: helpers.generator.slug(),
  ...
  static async userArticlePublished(req, res) {
    const { limit, offset } = req.query;
    const published = await Article.getUserArticles(
      parseInt(limit, 0) || 20,
      offset || 0,
      {
        userId: req.user.id,
  ...
  static async update(req, res) {
    ...
    return res.status(status.OK).send({
      message,
      article: req.method === 'DELETE' ? null : updatedArticle
    });
  }
  ...
  static async removeBookmarkOrFavorite(req, res) {
    ...
    return !result
      ? res.status(status.BAD_REQUEST).json({
        errors: {
          [resourceAction]: `article not removed from ${resourceAction}s`
        }
      })
      : res.status(status.OK).json({
        slug,
