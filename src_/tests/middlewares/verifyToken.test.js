...
describe('Verify token', () => {
  ...
  it('should return an unized status code if the token is blacklisted', (done) => {
    chai
      .request(app)
      .get('/api/users')
      .set('access-token', token)
      .end((err, res) => {
        expect(res.status).to.equal(status.UNIZED);
        expect(res.body).to.include.keys('errors');
        done();
      });
  });

  it('should return an unized status code if the token is not provided', (done) => {
    chai
      .request(app)
      .get('/api/users')
      .end((err, res) => {
  ...
  it('should return an unized status code the token is not valid', (done) => {
    chai
      .request(app)
      .get('/api/users')
