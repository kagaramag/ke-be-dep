...
describe('No articles', () => {
  ...
  describe('Test not-found articles', () => {
    ...
    it('Should not update if  does not exist', (done) => {
      delete article.id;
...
describe('Article', () => {
  ...
  before(async () => {
    ...
    // create gallery
    await db.Gallery.create({ image: 'placeholder.png', userId:  });
  });
  // create article
  ...
  it('Should trim empty space before the first letter of the title and after the last letter of the title', (done) => {
    ...
    const fakeArticle2 = { ...article, userId:  };
    chai
      .request(app)
      .post('/api/v1/articles')
      .send(fakeArticle2)
  ...
  it('Should trim empty spaces before the content of the article and after the last letter of the content', (done) => {
    ...
    const fakeArticle3 = { ...article, userId:  };
    chai
      .request(app)
      .post('/api/v1/articles')
      .set('access-token', accessToken)
  ...
  it('Should throw error if  filter is called more than once', (done) => {
    const  = 'text';
    chai
      .request(app)
      .get(`/api/v1/articles?=${}&=${}`)
      ...
      .end((err, res) => {
        ...
        res.body.errors..should.equal(' can not be declared more than once');
        done();
      });
  });
  ...
  it('Should not create article if the  does not exist', (done) => {
    delete article.id;
    delete article.slug;
    delete article.readTime;
    ...
    chai
      ...
      .end((err, res) => {
        expect(res).to.have.status(status.UNIZED);
        res.body.should.be.an('object');
