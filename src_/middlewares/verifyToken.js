...
export default async (req, res, next) => {
  ...
  if (!token) {
    return res.status(status.UNIZED).json({ errors: { authentication: 'Please, sign-in!' } });
  }
  ...
  if (!isLoggedout.errors && Object.keys(isLoggedout).length) {
    return res.status(status.UNIZED).json({ errors: { token: 'This token is invalid' } });
  }
